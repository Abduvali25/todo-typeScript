...
const App: React.FunctionComponent = () => {
  ...
  return (
    <div className="flex flex-col items-center bg-gray-900 rounded-lg p-6 w-full min-h-screen text-white ">
      <h1 className="text-3xl font-bold mb-4 mt-[100px]">Todo List</h1>
      <div className="flex flex-row justify-center items-center mb-4 w-full max-w-md p-[10px] ">
        <input
          type="text"
          placeholder="Add a new task"
          value={text}
          onChange={(event) => setText(event.target.value)}
          className="border border-gray-600 h-10 rounded-lg p-2 mr-2 todo-input"
        />
        <button
          id="insBtn"
          disabled={text === ""}
          onClick={addNewTodo}
          className="w-10 h-10 bg-purple-400 text-white rounded-lg flex items-center justify-center button"
        >
          +
        </button>
      </div>
            ...
            .map((t) => (
              ...
              >
                ...
                <button
                  ...
                  className="border-none rounded-lg h-8 absolute right-4 top-1/2 transform -translate-y-1/2 bg-purple-600 p-2 text-white button"
                >
                  ✓
                </button>
                <button
                  ...
                  className="border-none rounded-lg absolute right-6 top-1/2 transform -translate-y-1/2 bg-purple-600 p-2 text-white button"
                >
                  🗑️
                </button>
              </li>
